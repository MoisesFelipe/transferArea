...
class Dataset(threading.Thread):
    ...
    def _next_train(self):
        ...
        if self.image_batching:
            ...
            # print("img: "+str(self.images.shape))
            # print("image_index: "+str(image_index))
            # print("ray_indices: "+str(ray_indices))
    ...
    def _load_renderings(self, args):
        ...
        for imgfile in imgfiles:
            with utils.open_file(imgfile, "rb") as imgin:
                image = np.array(Image.open(imgin), dtype=np.float32) / 255.0
                # print("initial shape: "+str(image.shape[:2]))
                if args.factor > 1:
                    [halfres_h, halfres_w] = [hw // args.factor for hw in image.shape[:2]]
                    image = cv2.resize(
                        image, (halfres_w, halfres_h), interpolation=cv2.INTER_AREA
                    )
                
                # print("later shape: "+str(image.shape[:2]))
